#!/bin/csh -f
# Link the standard SDL2 OpenGL smoke.  Run the resulting binary only from a
# login session that can create an AppKit/DPS window.
...
set test_binary = /tmp/SDL20/bin/openstep-sdl-gl-window-smoke

if (! -r $archive || ! -r $mesa/lib/libGL.a) then
    echo "link-openstep-sdl-gl-window-smoke: build libSDL2.a and Mesa first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-gl-window-smoke: PASS $test_binary (not run)"
