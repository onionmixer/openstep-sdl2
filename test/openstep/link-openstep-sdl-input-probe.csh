...
set test_source = /tmp/SDL20/src/test/openstep/openstep-sdl-input-probe.c
set test_binary = /tmp/SDL20/bin/openstep-sdl-input-probe

if (! -r $build_root/SDL-init-closure-gate.o || ! -r $mesa/lib/libGL.a) then
    echo "link-openstep-sdl-input-probe: build the SDL closure and Mesa first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-input-probe: PASS $test_binary (run from GUI Terminal)"
