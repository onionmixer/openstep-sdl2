#!/bin/csh -f
# Link the standard SDL2 cursor/warp smoke; run it from a Workspace console.
set build_root = /tmp/SDL20/build/SDL-2.32.10-openstep
set mesa = /tmp/SDL20/mesa/Mesa-3.4.2
set test_source = /tmp/SDL20/src/test/openstep/openstep-sdl-cursor-smoke.c
set test_binary = /tmp/SDL20/bin/openstep-sdl-cursor-smoke

if (! -r $build_root/SDL-init-closure-gate.o) then
    echo "link-openstep-sdl-cursor-smoke: build the SDL closure first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-cursor-smoke: PASS $test_binary (not run)"
