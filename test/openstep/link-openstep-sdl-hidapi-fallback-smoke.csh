#!/bin/csh -f
# Link the public disabled-HIDAPI smoke against the real SDL closure.
...
set test_binary = /tmp/SDL20/bin/openstep-sdl-hidapi-fallback-smoke

if (! -r $build_root/SDL-init-closure-gate.o) then
    echo "link-openstep-sdl-hidapi-fallback-smoke: build the SDL closure first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-hidapi-fallback-smoke: PASS $test_binary (not run)"
