#!/bin/csh -f
# Link the standard AppKit window lifecycle smoke; run it only at the console.
...
set test_binary = /tmp/SDL20/bin/openstep-sdl-window-lifecycle-smoke

if (! -r $build_root/SDL-init-closure-gate.o) then
    echo "link-openstep-sdl-window-lifecycle-smoke: build the SDL closure first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-window-lifecycle-smoke: PASS $test_binary (not run)"
