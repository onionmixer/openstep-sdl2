#!/bin/csh -f
# Link the Return/Escape SDL message-box default-button probe.
set build_root = /tmp/SDL20/build/SDL-2.32.10-openstep
set mesa = /tmp/SDL20/mesa/Mesa-3.4.2
set test_source = /tmp/SDL20/src/test/openstep/openstep-sdl-messagebox-key-defaults-probe.c
set test_binary = /tmp/SDL20/bin/openstep-sdl-messagebox-key-defaults-probe

if (! -r $build_root/SDL-init-closure-gate.o) then
    echo "link-openstep-sdl-messagebox-key-defaults-probe: build the SDL closure first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-messagebox-key-defaults-probe: PASS $test_binary (not run)"
