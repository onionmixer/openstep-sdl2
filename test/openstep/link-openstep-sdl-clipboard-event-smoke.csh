#!/bin/csh -f
# Link the public SDL clipboard-update event test for the OPENSTEP backend.
...
set test_binary = /tmp/SDL20/bin/openstep-sdl-clipboard-event-smoke

if (! -r $build_root/SDL-init-closure-gate.o) then
    echo "link-openstep-sdl-clipboard-event-smoke: build the SDL closure first"
    exit 2
endif
...
if ($status != 0) exit 1
echo "link-openstep-sdl-clipboard-event-smoke: PASS $test_binary (not run)"
